...
class WeightedRegression:
    def __init__(self, weighting="distance"):
        ...
        self.y_train = None
    def fit(self, X, y):
        self.X_train = X; self.y_train = y; return self
...
class SVMClassifier:
    def __init__(self):
        self.best_kernel = None; self.best_model = None; self.kernel_scores = {}
    def fit_best(self, X_train, y_train):
        ...
        self.best_model  = SVC(kernel=self.best_kernel, C=1.0, gamma="scale",
                               probability=True, random_state=42, decision_function_shape="ovr")
        self.best_model.fit(X_train, y_train); return self
...
class ClusteringClassifier:
    ...
    def fit(self, X, y):
        ...
        return self
    def predict(self, X):  return np.array([self.cluster_label[c] for c in self.kmeans.predict(X)])
    def score(self, X, y): return accuracy_score(y, self.predict(X))

class DecisionTreeRuleLearner:
    def __init__(self, max_depth=6):
        self.tree = DecisionTreeClassifier(max_depth=max_depth, min_samples_split=10,
                                           min_samples_leaf=5, criterion="gini", random_state=42)
        self.feature_names = None
    def fit(self, X, y, feature_names):
        self.feature_names = feature_names; self.tree.fit(X, y); return self
    def predict(self, X):  return self.tree.predict(X)
    def score(self, X, y): return accuracy_score(y, self.predict(X))
    def get_rules(self):   return export_text(self.tree, feature_names=self.feature_names)
    def feature_importance_dict(self):
        return dict(zip(self.feature_names, self.tree.feature_importances_))

class RuleBasedClassifier:
    def __init__(self): self.rules = []; self.default = 0
    def _extract_rules(self, tree_clf):
        t, rules = tree_clf.tree_, []
        def recurse(node, conditions):
            if t.feature[node] == -2:
                rules.append((list(conditions), int(tree_clf.classes_[np.argmax(t.value[node])]))); return
    ...
    def fit(self, X, y, feature_names):
        ...
        self.default = int(vals[np.argmax(counts)]); self.feature_names = feature_names; return self
    ...
    def predict(self, X):
        ...
        for x in X:
            ...
            for conditions, label in self.rules:
                if self._apply_rule(x, conditions): preds.append(label); matched = True; break
            if not matched: preds.append(self.default)
        return np.array(preds)
    def score(self, X, y): return accuracy_score(y, self.predict(X))
